...
namespace Ninject.Web.AspNetCore.RequestActivation
{
	public class KeyedRequest : IRequest
	{
		...
		public KeyedRequest(Type service, object serviceKey, Func<IBindingMetadata, bool> constraint, IReadOnlyList<IParameter> parameters, Func<object> scopeCallback, bool isOptional, bool isUnique)
		{
			Service = service;
		...
		/// <summary>
		/// The specific servicekey to resovle this request for.
		/// </summary>
		...
		{
			return new KeyedRequest(parentContext, service, null, target, this.ScopeCallback);
		}
